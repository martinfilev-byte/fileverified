          ...
          <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-100 overflow-hidden mb-20">
             ...
             <div className="p-8 border-b bg-slate-50 flex justify-between items-center">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-black uppercase text-xs tracking-widest text-slate-800">Редактиране на Доклад</span>
                </div>
            </div>
            ...
            <div className="p-8 md:p-16 space-y-16">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <InputGroup label="Клиент" icon={<User size={18}/>}>
                  <input className="w-full p-5 bg-slate-50 rounded-2xl outline-none font-bold text-xl border-2 border-transparent focus:border-emerald-500 shadow-inner" placeholder="Име на клиент" value={editingReport.clientName} onChange={(e) => setEditingReport({...editingReport, clientName: e.target.value})} />
                </InputGroup>
              </div>
              ...
              <div className="bg-white rounded-[3rem] p-10 grid grid-cols-2 lg:grid-cols-4 gap-8 border border-slate-100 shadow-sm">
                  ...
                  <LightInput label="Пробег" value={editingReport.data.mileage} onChange={(v: string) => setEditingReport({...editingReport, data: {...editingReport.data, mileage: v}})} />
                  <LightInput label="Двигател" value={editingReport.data.engine} onChange={(v: string) => setEditingReport({...editingReport, data: {...editingReport.data, engine: v}})} />
                  <LightInput label="Мощност (hp)" value={editingReport.data.hp} onChange={(v: string) => setEditingReport({...editingReport, data: {...editingReport.data, hp: v}})} />
              ...
              <div className="bg-white border-2 border-slate-100 rounded-[3rem] p-10 space-y-6">
                  ...
                  <textarea className="w-full bg-slate-50 border border-slate-100 p-6 rounded-[2rem] outline-none min-h-[150px] font-medium text-slate-600" placeholder="Заключение от диагностиката..." value={editingReport.data.diagnostics} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setEditingReport({...editingReport, data: {...editingReport.data, diagnostics: e.target.value}})} />
              </div>

              <div className="space-y-4">
                  <h3 className="font-black uppercase text-sm tracking-[0.3em] text-emerald-600 ml-6">ЕКСПЕРТНО ЗАКЛЮЧЕНИЕ</h3>
                  <textarea className="w-full bg-emerald-600 text-white p-12 rounded-[4rem] outline-none font-black text-2xl shadow-2xl shadow-emerald-200 placeholder:text-emerald-300 min-h-[200px]" placeholder="Финална присъда..." value={editingReport.data.finalVerdict} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setEditingReport({...editingReport, data: {...editingReport.data, finalVerdict: e.target.value}})} />
              </div>
